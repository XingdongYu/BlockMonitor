...
class StackSampler {
    ...
    StackSampler(Thread thread, int maxEntryCount, long sampleIntervalMillis) {
    ...
    void printThreadStackEntries(long startTime, long endTime) {
        synchronized (sStackMap) {
            for (Long entryTime : sStackMap.keySet()) {
                if (startTime < entryTime && entryTime < endTime) {
                    Log.d(TAG, TIME_FORMATTER.format(entryTime)
                            + " takes "
                            + (endTime - startTime)
                            + "ms"
                            + SEPARATOR
